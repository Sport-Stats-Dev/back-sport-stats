...
exercise_api_blueprint = Blueprint('exercise_blueprint', __name__, static_folder='static', template_folder='templates')
...
@exercise_api_blueprint.route(Routes.EXERCISE_ROUTE, methods=['POST'])
def add_exercise():
...
@exercise_api_blueprint.route(Routes.EXERCISE_ROUTE, methods=['GET'])
def get_all_exercise():
    return jsonify(exercises_schema.dump(Exercise.query.all()))

@exercise_api_blueprint.route(Routes.EXERCISE_ROUTE + '/<id>', methods=['GET'])
def get_exercise_by_id(id):
    return exercise_schema.jsonify(Exercise.query.get_or_404(id))

@exercise_api_blueprint.route(Routes.EXERCISE_ROUTE + '/user_id=<user_id>', methods=['GET'])
def get_exercise_by_user_id(user_id):
    return exercise_schema.jsonify(Exercise.query.filter_by(user_id=user_id).first_or_404())

@exercise_api_blueprint.route(Routes.EXERCISE_ROUTE + '/<id>', methods=['PUT'])
...
@exercise_api_blueprint.route(Routes.EXERCISE_ROUTE + '/<id>', methods=['DELETE'])
